...
env_name = "env_noisy_A2C"


def test_stats():
    # --- CONFIGURAZIONE ---
    N_EPISODES = 500  # usa 100 per debug, 500-1000 per risultati “seri”
    VERSION = "v2"
    TIMESTEPS = 5000000
    ...
    print(f"--- INIZIO TEST STATISTICO A2C (Su {N_EPISODES} episodi) ---")
    ...
    for ep in range(N_EPISODES):
        ...
        while not done:
            ...
            # ricostruiamo la distanza drone-target come nell'env
            x = inner_env.x
            y = inner_env.y
            xt = inner_env.xt
            yt = inner_env.yt
    ...
    # --- 5. Risultati ---
    print("\n" + "=" * 50)
    print(f"RISULTATI TEST A2C ({N_EPISODES} EPISODI)")
