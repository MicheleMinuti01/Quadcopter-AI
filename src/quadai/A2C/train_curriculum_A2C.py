"""
Training A2C con curriculum learning sull'env noisy.

Fase 0: vento molto leggero, niente rumore sensori (quasi nominale)
Fase 1: vento medio, rumore sensori moderato
Fase 2: vento “full” come nel tuo env_noisy_A2C

Il modello A2C è sempre lo stesso: si continua a fare learn
...
def train_curriculum():
    ...
    env0 = DummyVecEnv(
        [
            lambda: Monitor(
                make_env(level=0),
                os.path.join(log_dir, "monitor_a2c_curr_phase0.csv"),
            )
        ]
    )
    ...
    env1 = DummyVecEnv(
        [
            lambda: Monitor(
                make_env(level=1),
                os.path.join(log_dir, "monitor_a2c_curr_phase1.csv"),
            )
        ]
    )
    ...
    env2 = DummyVecEnv(
        [
            lambda: Monitor(
                make_env(level=2),
                os.path.join(log_dir, "monitor_a2c_curr_phase2.csv"),
            )
        ]
    )
    ...
    print(f"\nModello A2C Curriculum salvato in: {final_path}")
